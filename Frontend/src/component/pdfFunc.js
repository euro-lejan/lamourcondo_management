...
async function printResident(data) {
  ...
  for (var key in data) {
    if (data.hasOwnProperty(key)) {
      ...
      row.push({
        text: header.firstname + " " + header.firstname,
        style: "tableItem",
      });
...
async function printMonth(data,month) {
  ...
  finalData.push(
    (dataBody[1] = await returnedTarget[0][month?month:''])
  );
  finalData.push(
    (dataBody[2] = await returnedTarget[1][month?month:''])
  );
  finalData.push(
...
async function printYear(data,year) {
  ...
  finalData.push(
    (dataBody[1] = await returnedTarget[0][year?year:''])
  );
  finalData.push(
    (dataBody[2] = await returnedTarget[1][year?year:''])
  );
  finalData.push(
